...
export default function MiPerfil() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      const { data: perfilData, error: perfilError } = await supabase
        .from("profiles")
        .select(
          "id, email, role, comunidad_id, created_at, username, full_name, avatar_url, telefono, piso"
        )
        .eq("id", user.id)
        .single();
      ...
      if (perfilData.comunidad_id) {
        try {
          const { data: comunidadData, error: comErr } = await supabase
            .from("comunidades")
            .select(
              "nombre, tipo, ciudad, num_vecinos, num_bloques, num_zonas_comunes"
            )
  ...
  /* ── Datos derivados ── */
  const rol = rolConfig[perfil.role] ?? {
    label: perfil.role,
              ...
              <div className="info-list">
                {[
                  ...
                  },
                  {
                    ...
                    label: "Miembro desde",
                    valor: formatFecha(perfil.created_at),
                  },
                  {
                    icono: "fa-solid fa-location-dot",
                    label: "Ciudad",
                    valor: ciudadMostrada,
                  },
                ].map((item, i) => (
              ...
              <div className="comunidad-info">
                ...
                <div className="comunidad-stats">
                  <div className="comunidad-stat">
                    <div className="comunidad-stat-val">
                      {comunidad.num_vecinos ?? "—"}
                    </div>
                    <div className="comunidad-stat-lbl">Vecinos</div>
                  </div>
                  <div className="comunidad-stat">
                    <div className="comunidad-stat-val">
                      {comunidad.num_bloques ?? "—"}
                    </div>
                    <div className="comunidad-stat-lbl">Bloques</div>
                  </div>
                  <div className="comunidad-stat">
                    <div className="comunidad-stat-val">
                      {comunidad.num_zonas_comunes ?? "—"}
                    </div>
            ...
            <div className="perfil-card-body">
              {actividad.length === 0 ? (
                ...
                <div className="actividad-list">
                  {actividad.map((a, i) => {
                    const cfg = iconoActividad[a.tipo] ?? {
                      clase: "act-chat",
          ...
          <div className="perfil-card">
            ...
            <div className="perfil-card-body">
              <div className="permisos-grid">
                {permisos.map((p, i) => (
                  <div
                    className={`permiso-chip ${p.activo ? "activo" : ""}`}
                    key={i}
                  >
                    <i className={p.icono} />
                    {p.label}
