...
export default function Chat() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (!data.user) setLoadingData(false);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const canal = supabase
      .channel(
        `chat-${[usuarioActual.id, vecinoSeleccionado.id].sort().join("-")}`
      )
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "messages" },
        (payload) => {
          ...
          const esDeEstaConv =
            (msg.sender_id === usuarioActual.id &&
              msg.receiver_id === vecinoSeleccionado.id) ||
            (msg.sender_id === vecinoSeleccionado.id &&
              msg.receiver_id === usuarioActual.id);
  ...
  return (
    <div className="bc-wrapper">
      ...
      {/* zona de mensajes */}
      <div className={`bc-chat${pantallaMovil === "lista" ? " oculto" : ""}`}>
            ...
            <>
              ...
              <div className="bc-chat-header">
                <button
                  ...
                  onClick={() => setPantallaMovil("lista")}
                >
                  ←
                </button>
                <div className="bc-avatar">
                ...
                <div>
                  <div className="bc-chat-header-name">
                    {getNombre(vecinoSeleccionado)}
                  </div>
                  <div className="bc-chat-header-rol">
                    {vecinoSeleccionado.role || "Vecino"}
                  </div>
                </div>
              </div>
              ...
              <ul className="bc-messages">
                {listaMensajes.length === 0 && (
                  <li className="bc-hello">
                    Di hola a {getNombre(vecinoSeleccionado)} 👋
                  </li>
                )}
                {listaMensajes.map((msg) => {
                  ...
                  return (
                    <li
                      key={msg.id}
                      className={`bc-msg-row${esMio ? " mio" : " otro"}`}
                    >
                      <div className={`bc-bubble${esMio ? " mio" : " otro"}`}>
                        {!esMio && (
                          <div className="bc-bubble-remitente">
                            {getNombre(vecinoSeleccionado)}
                          </div>
                        )}
                        {msg.content}
                        <div className="bc-bubble-hora">
                          {getFechaHora(msg.created_at)}
                        </div>
                      </div>
                    </li>
                  );
